...
HttpRequest::HttpRequest(const string& _request, int _serv_socket_communication_fd) {
	string			first_line;
	string			header;
	string			body;
	...
	is_complete = false;
	is_chunked = false;
	serv_socket_communication_fd = _serv_socket_communication_fd;
...
void	HttpRequest::display_request() {
	...
	cout << BG_MAGENTA << "Body: " << get_body() << RESET << "\n";
}


void	HttpRequest::set_server(Server _server) {
	this->server = _server;
}
...
bool	HttpRequest::get_is_complete() const {
...
int	HttpRequest::get_serv_socket_communication_fd() const {
	return (this->serv_socket_communication_fd);
}

streampos	HttpRequest::get_file_offset() const {
